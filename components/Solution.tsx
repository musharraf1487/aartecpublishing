    ...
    <section id="solution" className="py-24 px-6 bg-[#f0f4f4]">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-20">
          <span className="text-aartec-teal font-bold uppercase tracking-widest text-xs mb-4 block">The AARTEC Framework</span>
          <h2 className="text-4xl lg:text-5xl font-bold text-aartec-charcoal serif">The High-Ticket Ecosystem</h2>
        </div>

        <div className="grid lg:grid-cols-3 gap-8">
          {steps.map((item, idx) => (
            <div key={idx} className="bg-white rounded-3xl p-10 h-full shadow-sm hover:shadow-2xl transition-all duration-500 border-t-4 border-aartec-teal group">
              <div className="text-5xl mb-8 filter grayscale group-hover:grayscale-0 transition-all">{item.icon}</div>
              <div className="text-aartec-amber font-bold text-[10px] uppercase tracking-widest mb-3">{item.step}</div>
              <h3 className="text-2xl font-bold text-aartec-charcoal mb-2 serif">{item.title}</h3>
              <div className="text-aartec-teal text-xs font-semibold mb-6 tracking-wide italic">{item.subtitle}</div>
